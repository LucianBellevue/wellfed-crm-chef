...
import { BASE_URL, POST_MEDIA, GET_MEDIA, GET_RECEPIES_PAGE, PUT_MEDIA } from '@/constants/api';
...
import { DELETE_MEDIA } from '@/constants/api';
...
export default function MediaPage() {
  ...
  const deleteMedia = async (mediaId: string, fileName: string) => {  
    try {
      const res = await fetch(`${BASE_URL}${DELETE_MEDIA}${mediaId}`, {
        method: 'DELETE',
  ...
  const replaceMedia = async (mediaId: string, selectedFile: File) => {
    try {
      const formData = new FormData();
      formData.append('file', selectedFile);
      formData.append('userId', user?.uid || '');
      const res = await fetch(`${BASE_URL}${PUT_MEDIA}${mediaId}`, {
        method: 'PUT',
        body: formData,
      });
      if (!res.ok) throw new Error('Failed to replace media');
      setSuccess('Media replaced successfully');
      await fetchMediaFiles();
    } catch (err) {
    ...
    <div className="max-w-6xl mx-auto">
      ...
      {selectedMedia && (
        <MediaPreviewModal 
          ...
          onDelete={async () => deleteMedia(selectedMedia.id, selectedMedia.name)} 
          onReplace={(id, selectedFile) => replaceMedia(id, selectedFile)}
        />
      )}
              ...
              {files
                ...
                .map((file) => (
                  <MediaCard
                    ...
                    file={file}
                    onDelete={() => deleteMedia(file.id, file.name)}
